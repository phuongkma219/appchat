...
public class SecureChatSystem {
    ...
    public String decode(String payload, byte[] key) {
        //return payload;
        try {
           /* if (key != null && key.length > 0 && payload != null && payload.length() > 0) {
                byte[] data = cryptoLib.cbcGostDecrypt(RSAUtil.base64Decode(payload), key);
    ...
    public String encode(String mesage, byte[] key) {
       ...
       // return mesage;
        try {
           // byte[] data = cryptoLib.cbcGostEncrypt(mesage.getBytes("UTF-8"),key);
           byte[] data = AES.encrypt(mesage, key);
            return RSAUtil.base64Encode( data );
        } catch (Exception e) {
            e.printStackTrace();;
        }
        return "";
