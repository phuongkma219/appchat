...
public class CreateAccountUserNameFragment extends Fragment {
        ...
        api.registNewAccount(request).enqueue(new Callback<ApiResponse<AuthenResponse>>() {
            ...
            public void onResponse(Call<ApiResponse<AuthenResponse>> call, Response<ApiResponse<AuthenResponse>> response) {
                ...
                if (response.body() == null) {
                    Toast.makeText(CreateAccountUserNameFragment.this.getContext(), "Có lỗi xảy ra !!", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                Toast.makeText(CreateAccountUserNameFragment.this.getContext(), "Đăng ký thành công !", Toast.LENGTH_SHORT).show();
            }
    ...
    boolean validatePassword(String password) {
        if (password.isEmpty()) {
            lbErrorAccount.setText("* Mật khẩu không được trống !");
            return false;
        }
        if (password.length() < 6 || password.length() > 13) {
            lbErrorAccount.setText("* Mật khẩu phải dài từ 6 đến 13 ký tự !");
            return false;
        }
        return true;
    }
    ...
    boolean validateUserName(String username) {
        ...
        if (username.length() < 6 || username.length() > 13) {
            lbErrorAccount.setText("* Tên đăng nhập phải dài từ 6 đến 13 ký tự !");
            return false;
        }
        ...
        api.userExist(username).enqueue(new Callback<ApiResponse<UserInfo>>() {
            ...
            public void onResponse(Call<ApiResponse<UserInfo>> call, Response<ApiResponse<UserInfo>> response) {
                if (response.body() != null && response.body().error != 1 && response.body().data != null) {
                    lbErrorAccount.setText("* Tên đăng nhập đã tồn tại !");
                } else {
